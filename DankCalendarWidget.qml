...
PluginComponent {
    ...
    property string scriptPath: PluginService.pluginDirectory + "/danielCalendarAgenda/get-next-event"
    property string agendaScriptPath: PluginService.pluginDirectory + "/danielCalendarAgenda/get-agenda-events"
    ...
    function joinMeeting(link) {
        ...
        if (url) {
            hideEventTooltip();
            if (/^https:\/\/meet\.google\.com(?:[/?#]|$)/i.test(url)) {
